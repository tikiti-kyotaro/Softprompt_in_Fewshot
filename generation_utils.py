...
class GenerationMixin:
    ...
    def postprocess_next_token_scores(
        ...
        batch_size,
        num_beams,
    ):  # トークンに対してスコアづけして生成するトークンを予測？
        # repetition penalty (from CTRL paper https://arxiv.org/abs/1909.05858)
        if repetition_penalty != 1.0:  # ここには入らない
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.get_output_embeddings() is None:  # embedding はある
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder = self.get_encoder()  # encoder の指定
            # LISA CHANGED HERE  ここ変えたって！！！！！！！重要！！！！！！！（入ってないかも...）
            temp_past_key_values = model_kwargs['past_key_values'] if 'past_key_values' in model_kwargs else None  # model_kwargs という辞書の 'past_key_values' の値を temp_past_key_values に代入
            ...
            print(f'past_key_values のキー!!! : {temp_past_key_values[0].keys()}')  # past_key_valuesのキーを表示    
        ...
        if num_return_sequences > 1 or num_beams > 1:  # ここにも入らない
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            model_inputs = self.prepare_inputs_for_generation(
                input_ids, past=past, attention_mask=attention_mask, use_cache=use_cache, **model_kwargs
            )  # ここも大事そう kwargsの辞書に対してキーを入力  条件を満たすまで past を更新
            # print(model_inputs)

            outputs = self(**model_inputs, return_dict=True)  # ここが大事だけどよくわからない、model_inputsには生成時に指定されるパラメータの値が辞書として入っている
            print(type(outputs))
            next_token_logits = outputs.logits[:, -1, :]  # logits がスコアになる
            ...
            scores = self.postprocess_next_token_scores(
                ...
                num_beams=1,
            )  # next_token_logits がスコアになる

            # if model has past, then set the past variable to speed up decoding
            if "past_key_values" in outputs:  # ここで past
            ...
            cur_len = cur_len + 1  # ここで cur_len を更新(1個右に進める？) 
    ...
    #         if past is not None:  # ここで past_key_values を使っている
