...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if layer_past is not None:
            past_key, past_value = layer_past[0].transpose(-2, -1), layer_past[1]  # transpose back cf below  # まる
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)  # 入力を埋め込み
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)  # 入力の位置ベクトルを埋め込み
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True) for _ in range(config.n_layer)])  # Moduleのリスト
    ...
    def forward(
        self,
        input_ids=None,  # 入力文のID列
        past_key_values=None,  # prompt?
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        head_mask=None,
        inputs_embeds=None,  # 入力文の埋め込み？
        encoder_hidden_states=None,
        encoder_attention_mask=None,
        use_cache=None,
        ...
        **kwargs,
        ...
        use_cache = use_cache if use_cache is not None else self.config.use_cache  # gpt2-large-s3?
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        if input_ids is not None and inputs_embeds is not None:  # 入力するのはIDかベクトルのどっちか
            raise ValueError("You cannot specify both input_ids and inputs_embeds at the same time")
        elif input_ids is not None:
            input_shape = input_ids.size()
            input_ids = input_ids.view(-1, input_shape[-1])
            batch_size = input_ids.shape[0]
        elif inputs_embeds is not None:  # こっちに入りたい
        ...
        if inputs_embeds is None:
            inputs_embeds = self.wte(input_ids)  # どっちにしろここでwteを用いてinputs_embedsを生成
        position_embeds = self.wpe(position_ids)
        if token_type_ids is not None:
            token_type_embeds = self.wte(token_type_ids)
        else:
            token_type_embeds = 0
        hidden_states = inputs_embeds + position_embeds + token_type_embeds  # 隠れ層（入力の情報と位置の情報をもつ）
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):  # past_key_values が作用
            if output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states.view(*output_shape),)
            # blockはモジュールがいっぱい入っていて、layer_pastにはpast_key_valueが入っている
            outputs = block(
                hidden_states,
                layer_past=layer_past,
                attention_mask=attention_mask,
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = GPT2Model(config)  # ここに GPT2Model の埋め込みがある！！
