...
class PrefixTuning(GPT2PreTrainedModel):
    ...
    def __init__(self, config, model_gpt2, optim_prefix=False, preseqlen=5, use_infix=False, deep_param=False):
        ...
        self.n_embd = config.n_embd  # 1280

        # 属性を持っていたらモデルのパラメータを代入、持っていなかったらデフォルトの値にする
        ...
        if True:
            ...
            if self.lowdata and self.lowdata_token is not None:  # ここには入らない
                low_data_init = 3
            ...
            elif not deep_param and not self.init_shallow:  # ここに入る
                low_data_init = 0
                print('[Full prefix-tuning Setting :) ]')  # Prefix tuning の設定を満たしていることを確認
                self.input_tokens = torch.arange(self.preseqlen).long()  # 0~4までの等差数列
                self.wte = nn.Embedding(self.preseqlen, config.n_embd)  # preseqlen : 語彙サイズ(5)、n_embd : 次元数(1280)
                # config.n_embd は埋め込む単語のID?
                self.control_trans = nn.Sequential(
                    nn.Linear(config.n_embd, self.mid_dim),
                    nn.Tanh(),
                    nn.Linear(self.mid_dim, config.n_layer * 2 * config.n_embd))  # モデルを構築
                if self.use_infix:  # False
                    self.wte2 = nn.Embedding(self.preseqlen, config.n_embd)
                    self.get_prompt = self.get_prompt_p5_infix
                else:
                    self.get_prompt = self.get_prompt_p5  # get_prompt_p5 に移動
    ...
    def get_prompt_p5(self, control_code=None, gpt2=None, bsz=None):  # ここに入る
        input_tokens = self.input_tokens.unsqueeze(0).expand(bsz, -1).to(self.device)
        temp_control = self.wte(input_tokens) # 入力文の埋め込み？
        past_key_values = self.control_trans(temp_control) #bsz, seqlen, layer*emb  # モデルにエンベディング入力?
        bsz, seqlen, _ = past_key_values.shape  # bsz:1, seqlen:5
