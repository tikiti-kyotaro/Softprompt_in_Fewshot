...
# Copyright (c) 2018, NVIDIA CORPORATION.  All rights reserved./home/kyotaro/prompt-order/PrefixTuning/data/webnlg_challenge_2017/test
#
...
}  # MODEL_CLASSES で Model と　Tokenizer を設定
...
def read_classifySentiment_files(path, tokenizer):  # 入力文:正解の組み合わせが入っているリストを返す
    file_dict = []
    with open(path, 'r') as f:
        for line in f:
...
def make_prompt_vector(input_text, tokenizer, model):  # 入力文の埋め込みを取得
    text_index = tokenizer.encode(input_text,add_prefix_space=True)
    vector = model.transformer.wte.weight[text_index,:]
    return vector

def make_new_prompt_filetest(input_file, tokenizer, device, prompt, model):
    with open(input_file, "r") as in_:
        for i, context in enumerate(in_):
            new_encoded_prompt = tokenizer.encode(context, add_special_tokens=False, return_tensors="pt")  # 重要ポイント
            new_input_ids = new_encoded_prompt.to(device)
...
def remake_context_for_itis(context):
    context_list = context.split()
    sentence, label = " ".join(context_list[:-1]), context_list[-1]
    re_context = f'{sentence} it is {label}'
    return re_context

def remake_context_for_review_sentiment(context):
    context_list = context.split()
    sentence, label = " ".join(context_list[:-1]), context_list[-1]
    re_context = f'Reivew : {sentence} Sentiment : {label}'
    return re_context

def make_new_prompt(test_id, input_list, train_dict, tokenizer, device, prompt, model, k, mode):
    for i, context_id in enumerate(input_list[test_id]):
        if i < k:
            context = train_dict[context_id]
            
            if mode == "itis":
                context = remake_context_for_itis(context)
            elif mode == "review-sentiment":
                context = remake_context_for_review_sentiment(context)
...
def load_train_dict(train_dict_file):
    with open(train_dict_file, "rb") as in_:
        train_dict = pickle.load(in_)
    return train_dict

def load_context_id(context_id_file):
    with open(context_id_file, "rb") as in_:
        context_id_list = pickle.load(in_)
    return context_id_list

def test_kate(train_dict_file, context_id_file, tokenizer, device, prompt, model):
    train_dict = load_train_dict(train_dict_file)
    input_list = load_context_id(context_id_file)

    a = make_new_prompt(input_list, train_dict, tokenizer, device, prompt, model)
...
def main():
    ...
    parser.add_argument("--prefix", type=str, default="", help="Text added prior to input.")  # ここがソフトプロンプト？
    ...
    parser.add_argument("--test_path", type=str, default="/home/kyotaro/prompt-order/SST-2/new/used_data/test.txt", help="predict target")
    parser.add_argument("--context_path", type=str, help="context file name")
    parser.add_argument("--context_id_file", type=str, default="/home/kyotaro/prompt-order/data/knn-list.pkl", help="context id")
    parser.add_argument("--train_dict_file", type=str, default="/home/kyotaro/prompt-order/data/train_dict.pkl", help="predict target")
    parser.add_argument("--knn_num", type=int, default=4, help="knn num")
    parser.add_argument("--mode", type=str, default="no_template", help="")
    ...
    if args.tuning_mode == 'finetune':
        ...
        if args.prefixModel_name_or_path is not None:
            ...
            if args.prefix_mode == 'embedding':
                ...
                model = PrefixTuning.from_pretrained(
                    ...
                    from_tf=bool(".ckpt" in args.prefixModel_name_or_path, ),
                    config=config,
                    model_gpt2=model, optim_prefix=optim_prefix_bool, preseqlen=args.preseqlen,
                    use_infix=(args.format_mode == 'infix')
                )
        ...
        model = model_class.from_pretrained(args.model_name_or_path, config=config, cache_dir=args.cache_dir)
        gpt2_param = list(model.parameters())
        print("gpt2_param に gpt2 のパラメータ入力")
        # print(gpt2_param)
        model.to(args.device)

        input_text = "It is a pen ."  # ここには example を入れる必要あり
        input_vec = make_prompt_vector(input_text=input_text, model=model, tokenizer=tokenizer)

        print(len(tokenizer), tokenizer.bos_token, tokenizer.eos_token, tokenizer.pad_token)  # 50257 <|endoftext|> <|endoftext|> None

        # TODO LISA
        add_pad = False

        if args.model_name_or_path == 'gpt2-large':  # 元々 medium で large　に変えた
        ...
        if args.prefixModel_name_or_path is not None:
            ...
            #
            config = AutoConfig.from_pretrained(args.prefixModel_name_or_path, cache_dir=args.cache_dir )
            #config = AutoConfig.from_pretrained(args.prefixModel_name_or_path, cache_dir=cache_dir )
            print(f'wanted!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!:{args.prefixModel_name_or_path}')
            ...
            if args.prefix_mode == 'embedding':
            ...
            elif args.prefix_mode == 'activation':   # model 読み込みポイント？
                ...
                # ここより下の model は生成したモデル
                prefix_param = list(model.parameters())  #  prefix model のパラメータ
                print("prefix_param に prefix_model のパラメータを入力")
    ...
    if args.task_mode == 'data2text':
        ...
        if args.task_mode == 'webnlg':
            if args.eval_dataset == 'valid':
                ...
                test_path = "/home/kyotaro/prompt-order/PrefixTuning/data/webnlg_challenge_2017/dev.json"
            elif args.eval_dataset == 'test':
                #test_path = "/u/scr/xlisali/WebNLG/webnlg-dataset/webnlg_challenge_2017/test.json"
                test_path = "/home/kyotaro/prompt-order/PrefixTuning/data/webnlg_challenge_2017/test.json"
            else:
        ...
        if QUICK_CHECK:
            ...
            #                         args.gen_dir,
            #                         '{}_{}_{}'.format(temp, split_file, decode_mode))
            curr_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}_{}'.format(temp, split_file, decode_mode, "remake"))  # 変更点
            print(curr_dir)
            # gold_dir = os.path.join('/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/',
            #                         args.gen_dir,
            #                         '{}_{}_{}'.format(temp, split_file, 'gold'))
            gold_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'gold_remake'))   # 変更点
            print(gold_dir)
            write_e2e_corr(prompt_text_pair, prompt_text_dict, gold_dir)
            # src_dir = os.path.join('/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/',
            #                         args.gen_dir,
            #                         '{}_{}_{}'.format(temp, split_file, 'src'))
            src_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'src_remake'))    # 変更点
        ...
        if args.task_mode == 'classify-sentiment':
            ...
            # test_path = "/home/kyotaro/prompt-order/SST-2/new/itis/itis_test.txt"
            # test_path = "/home/kyotaro/prompt-order/SST-2/new/review_sentiment/review_sentiment_test.txt"
            prompt_text_dict = read_classifySentiment_files(args.test_path, tokenizer)
        ...
        if QUICK_CHECK:
            ...
            #UNCHECKED
            prompt_text_lst, prompt_text_tgt = zip(*prompt_text_dict)  # prompt_text_lst : 入力文、prompt_text_tgt : 正解ラベル
            if args.prefixModel_name_or_path is not None:
                temp = os.path.basename(args.prefixModel_name_or_path)
            else:
                temp = os.path.basename(args.model_name_or_path)
            ...
            # curr_dir = os.path.join('/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/',
            #                         args.gen_dir,
            #                         '{}_{}_{}'.format(temp, split_file, decode_mode))  # 変更点
            curr_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))  # 変更点
            ...
            # gold_dir = os.path.join('/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/',
            #                         args.gen_dir,
            #                         '{}_{}_{}'.format(temp, split_file, 'gold'))  # 変更点
            gold_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                        args.gen_dir,
                        '{}_{}_{}'.format(temp, split_file, 'gold_test'))  # 変更点
            # gold_dir = '/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/classify_results/{}_{}_{}'.format(
            #     temp,
            #     split_file,
            #     'gold')
            print(gold_dir)
            write_e2e_src(prompt_text_tgt, gold_dir)  # 正解ラベルをgoldに記入
            # src_dir = os.path.join('/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/',
            #                        args.gen_dir,
            #                        '{}_{}_{}'.format(temp, split_file, 'src'))
            src_dir = os.path.join('/home/kyotaro/prompt-order/PrefixTuning/text-generation/',
                                   args.gen_dir,
                                   '{}_{}_{}'.format(temp, split_file, 'src_test'))  # 変更点
            # src_dir = '/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/classify_results/{}_{}_{}'.format(
            #     temp,
            #     split_file,
            #     'src')
            write_e2e_src(prompt_text_lst, src_dir)  # srcファイルに入力文を記入
            out_handle = open(curr_dir, 'w')
    ...
    # # print(input_ids)
    # # 重要ポイント！！　ここの prompt が prefix に該当する部分？
    # prompt = model.get_prompt(src, None, gpt2=gpt2, bsz=1) 
    ...
    # with open(args.context_path, "r") as in_:
    ...
    # knn の結果と train データのIDの辞書をロード
    context_id = load_context_id(args.context_id_file)
    train_dict = load_train_dict(args.train_dict_file)

    # print(f'context_id : {context_id}')
    # print(f'context_id : {len(context_id[0])}')
    # print(f'train_dict : {train_dict[0]}')
    # print(f'train_dict : {train_dict[1]}')



    for prompt_idx, prompt_text in enumerate(prompt_text_lst):  # pompt_idx が入力文のID, prompt_text が入力文
        # Different models need different input formatting and/or extra arguments
        # print(f'context_id {prompt_idx} : {context_id[prompt_idx]}')
        requires_preprocessing = args.model_type in PREPROCESSING_FUNCTIONS.keys()  # False
        if requires_preprocessing:
            ...
            encoded_prompt = tokenizer.encode(prefix + prompt_text, add_special_tokens=False, return_tensors="pt")  # 重要ポイント
        ...
        # 入力文の埋め込みを入手
        inputs_embeds = gpt2.transformer.wte.weight[input_ids,:]
        # print(inputs_embeds[0][7])
        ...
        if args.tuning_mode == 'prefixtune' or args.tuning_mode == 'bothtune':
            ...
            if decode_mode == 'nucleus':
                ...
                # ここで　KATE 
                new_prompt = make_new_prompt(prompt_idx, context_id, train_dict, tokenizer, args.device, prompt, gpt2, args.knn_num, args.mode)
    ...
    if args.task_mode == 'data2text':
        ...
        os.system("bash /home/kyotaro/prompt-order/dart/evaluation/run_eval_on_webnlg.sh "
                  "{} {} >> {}".format(curr_dir, tagging, out_file_eval))  # 変更点


    elif 'classify' in curr_dir:
        print(curr_dir)
        print(gold_dir)
        context_sign = f'{args.mode}_{args.knn_num}'
        temp_command = "python3 /home/kyotaro/prompt-order/eval/sst-2_eval.py {} {} {}".format(gold_dir, curr_dir, context_sign)
        print(temp_command)
