...
def main():
    ...
    if 'lowdata' in training_args.output_dir:
    ...
    elif data_args.task_mode == 'classify-sentiment':  # 変更点　classify-sentiment でも gen.py を使えるようにする
        del model
        del trainer
        if model_args.tuning_mode == 'prefixtune':
            del gpt2
        ...
        print('running evaluation on ', checkpoint_path)
        print('python gen.py classify-sentiment yes valid {} no'.format(checkpoint_path))
        os.system('python gen.py classify-sentiment yes valid {} no'.format(checkpoint_path))
        os.system('python gen.py classify-sentiment yes test {} no'.format(checkpoint_path))
